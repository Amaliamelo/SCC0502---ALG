...
int inserir(t_lista *lista, t_elemento elemento){
	...
	t_apontador novo = (t_apontador) malloc(sizeof(t_no)); //nova elemento
	
	if(novo==NULL)
		return ERRO CHEIA;
	

	novo->elemento = elemento; // novo apontando para elemento

	novo->proximo = lista->primeiro; //novo aponta para proxima
	lista->primeiro = novo; // primeiro aponta para novo
